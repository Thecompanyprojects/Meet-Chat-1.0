...
- (void)setUpMainView
{
    ...
    UIPanGestureRecognizer *minSliderButtonPanGestureRecognizer = [[UIPanGestureRecognizer alloc]initWithTarget:self action:@selector(panMinSlider:)];
    ...
    UIPanGestureRecognizer *maxSliderButtonPanGestureRecognizer = [[UIPanGestureRecognizer alloc]initWithTarget:self action:@selector(panMaxSlider:)];
...
- (void)panMinSlider:(UIPanGestureRecognizer *)pgr
{
    ...
    [self solvevalueMinChange:pgr.view.right];
}

- (void)panMaxSlider:(UIPanGestureRecognizer *)pgr
{
    CGPoint point = [pgr translationInView:self];
    static CGPoint center;
    if (pgr.state == UIGestureRecognizerStateBegan) {
    ...
    [self solvevalueMaxChange:pgr.view.xy_x];
}

- (void)solvevalueMinChange:(CGFloat)num
{
    _minLabel.text = [NSString stringWithFormat:@"%.3f%@",_minNum + (_tatol * (num-20.0)),_unit];
    _currentMinValue = _minNum + (_tatol * (num-20.0));
    if (self.delegate && [self.delegate respondsToSelector:@selector(ageSliderValueChange:)]) {
        [self.delegate ageSliderValueChange:self];
    }
}

- (void)solvevalueMaxChange:(CGFloat)num
{
