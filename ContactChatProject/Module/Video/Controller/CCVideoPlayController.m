...
@interface CCVideoPlayController ()
...
@implementation CCVideoPlayController
...
{
    if ([CCIsLodingManager sharedClient].isTouch) {
        self.isPause = !self.isPause;
        if (self.isPause) {
            //[self.videoplayer changePauseClick];
...
-(void)sayhibtnClick
{
    ...
    if (self.issayHi!=YES) {
        ...
        [self CClogManagersayHi];
        if (self.isfromDis) {
            [[ChatManager sharedChatManager] chatSayHi:botid withContent:content withUserName:self.disModel.name withPhoto:self.disModel.photo.firstObject];
        }
        else
        {
