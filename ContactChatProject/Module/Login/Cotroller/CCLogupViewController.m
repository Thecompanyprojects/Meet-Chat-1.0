...
@interface CCLogupViewController ()<YBAttributeTapActionDelegate>
...
@implementation CCLogupViewController
...
-(UIButton *)boyBtn
{
    if(!_boyBtn)
    {
        ...
        [_boyBtn setImage:[UIImage imageNamed:@"nan"] forState:normal];
...
-(UIButton *)girlBtn
{
    if(!_girlBtn)
    {
        ...
        [_girlBtn setImage:[UIImage imageNamed:@"nv2"] forState:normal];
...
-(UIButton *)submitBtn
{
    if(!_submitBtn)
    {
        ...
        [_submitBtn addTarget:self action:@selector(CCsubmitbtlcikck) forControlEvents:UIControlEventTouchUpInside];
...
-(UIButton *)leftBtn
{
    if(!_leftBtn)
    {
        ...
        [_leftBtn addTarget:self action:@selector(CCleftbtnClick) forControlEvents:UIControlEventTouchUpInside];
...
//    [[XYLogManager shareManager] addLogKey1:@"sign_up" key2:@"click" content:@{@"result":@(3)} userInfo:[NSDictionary new] upload:YES];
    ...
    [self.boyBtn setImage:[UIImage imageNamed:@"nan2"] forState:normal];
    [self.girlBtn setImage:[UIImage imageNamed:@"nv2"] forState:normal];
...
- (void)yb_attributeTapReturnString:(NSString *)string range:(NSRange)range index:(NSInteger)index
{
    ...
    if ([string isEqualToString:@"Terms of Servoce"]) {
        //terms_of_services = @"https://sites.google.com/view/teddyhookup/";
        terms_of_services = @"https://candywood.weebly.com/term-of-service.html";
    }else{
        terms_of_services = @"https://sites.google.com/view/teddyhookup-tos/";
    }
