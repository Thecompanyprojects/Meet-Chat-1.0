...
@interface CCSettingViewController ()<UITableViewDataSource,UITableViewDelegate>
...
-(UIButton *)logoutBtn
{
    if(!_logoutBtn)
    {
        ...
        [_logoutBtn addTarget:self action:@selector(CClogoutbtnClick) forControlEvents:UIControlEventTouchUpInside];
    }
    return _logoutBtn;
}
...
{
    CCSettingCell *cell = [tableView dequeueReusableCellWithIdentifier:settingIdentfity];
    cell = [[CCSettingCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:settingIdentfity];
...
-(void)CCshareClick
{
    NSString *textToShare = @"Meet&Chat";
    NSString * url = [NSString stringWithFormat:@"itms-apps://itunes.apple.com/WebObjects/MZStore.woa/wa/viewSoftware?id=%@",kAppStoreID];
...
-(void)CClogoutbtnClick
{
    ...
    UIAlertAction *action1 = [UIAlertAction actionWithTitle:@"OK" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        CCLoginViewController *main = [[CCLoginViewController alloc]init];
        CCloginnavController *nav = [[CCloginnavController alloc] initWithRootViewController:main];
