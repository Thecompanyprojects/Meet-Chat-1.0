...
#import "CCEditorheaderView.h"
#import "CCEditorCell0.h"
#import "CCEditorCell1.h"
#import "CCEditorCell2.h"
#import "CCConstellationJudge.h"
#import "CCEditoralertView.h"
#import "CGXPickerViewManager.h"
#import "CGXPickerView.h"

@interface CCEditorViewController ()<UITableViewDataSource,UITableViewDelegate,myTabVdelegate>
@property (nonatomic,strong) UITableView *table;
@property (nonatomic,strong) CCEditorheaderView *headerView;
...
@implementation CCEditorViewController
...
-(UIButton *)saveBtn
{
    if(!_saveBtn)
    {
        ...
        [_saveBtn addTarget:self action:@selector(CCsavebtnClick) forControlEvents:UIControlEventTouchUpInside];
    }
    return _saveBtn;
}


-(CCEditorheaderView *)headerView
{
    if(!_headerView)
    {
        _headerView = [[CCEditorheaderView alloc] init];
        _headerView.delegate = self;
        _headerView.frame = CGRectMake(0, 0, kScreenWidth, kScreenWidth);
        [_headerView.imgView0.addBtn addTarget:self action:@selector(CCimgchooseClick) forControlEvents:UIControlEventTouchUpInside];
        [_headerView.imgView1.addBtn addTarget:self action:@selector(CCimgchooseClick) forControlEvents:UIControlEventTouchUpInside];
        [_headerView.imgView2.addBtn addTarget:self action:@selector(CCimgchooseClick) forControlEvents:UIControlEventTouchUpInside];
        [_headerView.imgView3.addBtn addTarget:self action:@selector(CCimgchooseClick) forControlEvents:UIControlEventTouchUpInside];
        [_headerView.imgView4.addBtn addTarget:self action:@selector(CCimgchooseClick) forControlEvents:UIControlEventTouchUpInside];
        [_headerView.imgView5.addBtn addTarget:self action:@selector(CCimgchooseClick) forControlEvents:UIControlEventTouchUpInside];
    }
    return _headerView;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section==0) {
        CCEditorCell0 *cell = [tableView dequeueReusableCellWithIdentifier:editoridentfity0];
        cell = [[CCEditorCell0 alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:editoridentfity0];
        cell.model = self.personModel;
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        return cell;
    }
    if (indexPath.section==1) {
        CCEditorCell1 *cell = [tableView dequeueReusableCellWithIdentifier:editoridentfity1];
        cell = [[CCEditorCell1 alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:editoridentfity1];
    ...
    if (indexPath.section==2) {
        CCEditorCell2 *cell = [tableView dequeueReusableCellWithIdentifier:editoridentfity3];
        cell = [[CCEditorCell2 alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:editoridentfity3];
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    [self logeditManager];
    if (indexPath.section==0) {
        CCEditoralertView *alert = [[CCEditoralertView alloc] init];
    ...
    if (indexPath.section==2&&indexPath.row==1)
    {
        ...
        [CGXPickerView showDatePickerWithTitle:@"Birthday" DateType:UIDatePickerModeDate DefaultSelValue:nil MinDateStr:@"1900-01-01" MaxDateStr:nowStr IsAutoSelect:NO Manager:manager ResultBlock:^(NSString *selectValue) {
            ...
            self.personModel.horoscope =  [CCConstellationJudge calculateConstellationWithMonth:[month integerValue] day:[day integerValue]];
...
-(void)CCsavebtnClick
{
    ...
    if (self.personModel.birthday.length==0) {
        dic = @{@"name":self.personModel.name?:@"",@"id":[CCUserModel sharedUserModel].userId?:@"",@"signature":self.personModel.signature?:@"",@"city":self.personModel.city?:@"",@"single":self.single?:@([self.single intValue])};
    }
    else
    {
        dic = @{@"name":self.personModel.name?:@"",@"id":[CCUserModel sharedUserModel].userId?:@"",@"birthday":self.personModel.birthday?:@"",@"signature":self.personModel.signature?:@"",@"city":self.personModel.city?:@"",@"single":self.single?:@([self.single intValue])};
    }
