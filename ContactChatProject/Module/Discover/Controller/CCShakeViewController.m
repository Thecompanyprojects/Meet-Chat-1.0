...
#import "XTVCchoose.h"
#import "CCdiscoverAlertView.h"

@interface CCShakeViewController ()<CAAnimationDelegate> {
...
@property (nonatomic, strong) CCDiscoverModel *cacheModel;
@end

#define KKSHAKEMODEL @"infoDic"

@implementation CCShakeViewController
...
-(UIImageView *)shakeImg
{
    if(!_shakeImg)
    {
        ...
        _shakeImg.image = [UIImage imageNamed:@"摇一摇"];
...
-(void)CCshankClick
{
    ...
    self.cacheModel = [[CCDiscoverModel alloc] init];
    [[AFNetAPIClient sharedClient] requestUrl:Shake cParameters:dic success:^(NSDictionary * _Nonnull requset) {
        if ([[requset objectForKey:@"code"] intValue]==1) {
            ...
            if ([[XTVCchoose sharedClient].getCurrentVC isMemberOfClass:[CCShakeViewController class]]){
                // 匹配成功声音
                ...
                self.cacheModel = [CCDiscoverModel yy_modelWithJSON:botinfo];
                [self CCAddAlertViewtoView];
            }
            else
            {
                self.isCan = YES;
...
-(void)CCAddAlertViewtoView
{
    CCdiscoverAlertView *alertView = [CCdiscoverAlertView new];
    self.isCan = NO;
