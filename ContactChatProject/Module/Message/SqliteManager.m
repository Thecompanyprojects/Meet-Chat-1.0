...
- (CreatTable *)_firstCreatChatLogQueueWithType:(DBChatType)type sessionID:(NSString *)sessionID{
    ...
    if (queue) {
        ...
        NSString *creatTableSql = [CCMessageItem yh_sqlForCreatTable:tableName primaryKey:@"id"];
        if (creatTableSql) {
...
//更新多条聊天信息
- (void)updateChatLogWithType:(DBChatType)type sessionID:(NSString *)sessionID chatLogList:(NSArray <CCMessageItem *>*)chatLogList complete:(void (^)(BOOL success,id obj))complete{
    ...
    for (int i= 0; i< chatLogList.count; i++) {
        ...
        CCMessageItem *model = chatLogList[i];
...
//更新某条聊天信息
- (void)updateOneChatLogWithType:(DBChatType)type sessionID:(NSString *)sessionID aChatLog:(CCMessageItem*)aChatLog updateItems:(NSArray <NSString *>*)updateItems complete:(void (^)(BOOL success,id obj))complete{
...
- (void)queryChatLogTableWithType:(DBChatType)type sessionID:(NSString *)sessionID userInfo:(NSDictionary *)userInfo fuzzyUserInfo:(NSDictionary *)fuzzyUserInfo complete:(void (^)(BOOL success,id obj))complete{
    ...
    [queue inDatabase:^(FMDatabase *db) {
        [db yh_excuteDatasWithTable:tableNameChatLog(sessionID) model:[CCMessageItem new] userInfo:userInfo fuzzyUserInfo:fuzzyUserInfo otherSQL:nil option:^(NSMutableArray *models) {
            complete(YES,models);
        }];
    }];
}
...
//查询多条聊天信息
- (void)queryChatLogWithType:(DBChatType)type sessionID:(NSString *)sessionID list:(NSArray<CCMessageItem *>*)chatLogList complete:(void (^)(BOOL, id))complete{
    
    CreatTable *model = [self _setupDBqueueWithType:type sessionID:sessionID];
    FMDatabaseQueue *queue = model.queue;
    
    __block NSMutableArray *maRet = [NSMutableArray new];
    for (CCMessageItem *model in chatLogList) {
        [queue inDatabase:^(FMDatabase *db) {
...
//查询一条聊天信息
- (void)queryaChatLogWithType:(DBChatType)type sessionID:(NSString *)sessionID aChatLog:(CCMessageItem *)aChatLog userInfo:(NSDictionary *)userInfo complete:(void (^)(BOOL, id))complete{
...
//删除聊天信息数组
- (void)deleteChatLogWithType:(DBChatType)type sessionID:(NSString *)sessionID list:(NSArray <CCMessageItem *>*)chatLogList complete:(void(^)(BOOL success,id obj))complete;{
...
- (void)deleteOneChatLogWithType:(DBChatType)type sessionID:(NSString *)sessionID msgID:(NSString *)msgID complete:(void(^)(BOOL success,id obj))complete{
    ...
    CCMessageItem *model = [CCMessageItem new];
    model.chatId       = msgID;
...
//删除某一聊天信息
- (void)deleteaChatLogWithType:(DBChatType)type sessionID:(NSString *)sessionID aChatLog:(CCMessageItem *)aChatLog userInfo:(NSDictionary *)userInfo complete:(void(^)(BOOL success,id obj))complete{
