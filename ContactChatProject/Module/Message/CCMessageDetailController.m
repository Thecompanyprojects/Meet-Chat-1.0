...
#import "CCMessageTimeCell.h"
#import "SqliteManager.h"
#import "ChatSendManager.h"
#import "AppDelegate.h"
#import <MJRefresh.h>
#import "CCMessageImageCell.h"
...
@interface CCMessageDetailController ()
<UITableViewDelegate, UITableViewDataSource, UITextViewDelegate,CellPassDelegate>
...
- (void)viewDidLoad {
    ...
    [self.kTableView registerNib:[UINib nibWithNibName:NSStringFromClass([CCReciveMsgCell class]) bundle:nil] forCellReuseIdentifier:kCellID0];
    [self.kTableView registerNib:[UINib nibWithNibName:NSStringFromClass([CCSendMsgCell class]) bundle:nil] forCellReuseIdentifier:kCellID1];
    ...
    [[ChatManager sharedChatManager] cornerMarkZero:self.msgItem.userId];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(getMessageDataSource) name:kPaymentSuccessNotificationName object:nil];
...
-(void)getChatUserInfo{
    ...
    [[AFNetAPIClient sharedClient] requestUrl:getbotinfo cParameters:@{@"botid":@([self.msgItem.userId integerValue])} success:^(NSDictionary * response) {
        ...
        if ([[response objectForKey:@"code"] intValue]==1) {
            weakSelf.boltModel = [[CCDiscoverModel alloc] init];
...
-(void)getMessageDataSource{
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        NSString * sessionId = [NSString stringWithFormat:@"%@_%@",[CCUserModel sharedUserModel].userId,weakSelf.msgItem.userId];
        [[SqliteManager sharedInstance] queryChatLogTableWithType:DBChatType_Private sessionID:sessionId userInfo:nil fuzzyUserInfo:nil complete:^(BOOL success, id obj) {
            for (CCMessageItem * item in obj) {
                if (item.createDate - weakSelf.lastTime > TimeInterval) {
                    NSString * time = [NSString stringWithFormat:@"%@    ",[self getChateTime:item.createDate]];
...
//机器人聊天
-(void)requestChatContent:(NSString *)content withType:(NSInteger)msgType messageItem:(CCMessageItem *)item indexpath:(NSIndexPath *)indexpath{
                                          ...
                                          if ([[response objectForKey:@"code"] isEqual:@1]) {
                                              ...
                                              if ([data isKindOfClass:[NSDictionary class]]) {
                                                  if ([[data objectForKey:@"replyflag"] isEqual:@1]) {
                                                      ...
                                                      CCMessageItem * msgItem = [CCMessageItem new];
...
//                                                          [[ChatDownload sharedChatManager].downloadModelsDic setObject:@1 forKey:key];
//                                                      }
//
                                                      item.sendError = 0;
...
-(void)updateSendError:(CCMessageItem *)item indexpath:(NSIndexPath *)indexpath{
...
-(void)updateChatDetail:(CCMessageItem *)msgItem{
    
    NSString * sessionId = [NSString stringWithFormat:@"%@_%@",[CCUserModel sharedUserModel].userId,self.msgItem.userId];
    ...
    if (msgItem.createDate - self.lastTime > TimeInterval) {
        NSString * time = [NSString stringWithFormat:@"%@    ",[self getChateTime:msgItem.createDate]];
...
//更新消息详情
-(void)updateChatDetail:(CCMessageItem *)msgItem withAfterSecond:(NSInteger)sencond{
...
-(NSString *)getChateTime:(long long)timeStamp{
...
-(void)recieveData:(NSNotification *)user{
    ...
    CCMessageItem * item = [user.userInfo objectForKey:@"data"];
...
- (void)sendMessage:(NSString *)text autoAnswer:(BOOL)autoAnswer{
    ...
    CCMessageItem * msgItem = [CCMessageItem new];
    msgItem.userId = [CCUserModel sharedUserModel].userId;
    msgItem.photo = [CCUserModel sharedUserModel].userphotos.firstObject;
    msgItem.chatId = [NSString stringWithFormat:@"%f", [NSDate date].timeIntervalSince1970 * 1000];
    msgItem.message = text;
    msgItem.createDate = [NSDate date].timeIntervalSince1970 * 1000;
    msgItem.userName = [CCUserModel sharedUserModel].name;
    msgItem.sendUserId = [CCUserModel sharedUserModel].userId;
...
-(void)sendeMessage:(long long)date withText:(NSString *)text messageItem:(CCMessageItem *)item{
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(5.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        if (![self.messageDict objectForKey:[NSString stringWithFormat:@"%lld",date]]) {
            [self requestChatContent:text withType:1 messageItem:item indexpath:nil];
        }
    });
}
    ...
    [imagePickerVc setDidFinishPickingPhotosHandle:^(NSArray<UIImage *> *photos, NSArray *assets, BOOL isSelectOriginalPhoto) {
        if (photos.count!=0) {
            ...
            if (path) {
                ...
                msgItem.sendUserId = [CCUserModel sharedUserModel].userId;
                [self updateChatDetail:msgItem withAfterSecond:0];
                [self requestChatContent:@"" withType:2 messageItem:msgItem indexpath:nil];
            }
        }
    }];
            ...
            [[TZImageManager manager] getVideoWithAsset:phAsset completion:^(AVPlayerItem *playerItem, NSDictionary *info) {
                ...
                [[PHImageManager defaultManager] requestAVAssetForVideo:phAsset options:nil
                                                          resultHandler:^(AVAsset * _Nullable asset, AVAudioMix * _Nullable audioMix, NSDictionary * _Nullable info) {
                                                              ...
                                                              msgItem.sendUserId = [CCUserModel sharedUserModel].userId;
                                                              [self updateChatDetail:msgItem withAfterSecond:0];
                                                              [self requestChatContent:@"" withType:3 messageItem:msgItem indexpath:nil];
...
-(BOOL)isReceive{
    ...
    for (id  obj in self.msgMuArray) {
        if ([obj isKindOfClass:[CCMessageItem class]]) {
            CCMessageItem * item = (CCMessageItem *)obj;
            if ([item.userId isEqual:self.msgItem.userId]) {
                isRecieve = YES;
            }
        }
...
-(void)cellPassIndexPath:(NSIndexPath *)indexpath withObject:(id)object{
    if ([object isKindOfClass:[CCMessageItem class]]) {
        CCMessageItem * item = (CCMessageItem *)object;
        [self requestChatContent:item.message withType:item.msgType+1 messageItem:item indexpath:indexpath];
    }else{
...
-(void)cellPassIndexPath:(NSIndexPath *)indexpath withObject:(id)object withCell:(id)cell{
    CCMessageItem * item = self.dataArray[indexpath.row];
    if (item.msgType == 0) {
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    }else if([item isKindOfClass:[CCMessageItem class]]){
        CCMessageItem * msgItem = (CCMessageItem *)item;
        if (msgItem.msgType == 1) {
            //用户自己发送
            if ([msgItem.userId integerValue] == [[CCUserModel sharedUserModel].userId integerValue]) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    id item = self.dataArray[indexPath.row];
    if ([item isKindOfClass:[NSString class]]) {
        CCMessageTimeCell *timeCell = (CCMessageTimeCell *)[tableView dequeueReusableCellWithIdentifier:NSStringFromClass([CCMessageTimeCell class])];
        if (!timeCell) {
            timeCell = [[CCMessageTimeCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:NSStringFromClass([CCMessageTimeCell class])];
        }
        ...
        CCMessageItem *msgItem = (CCMessageItem *)item;
        if ([msgItem.userId integerValue] == [[CCUserModel sharedUserModel].userId integerValue]) {
            CCSendMsgCell *cell = [tableView dequeueReusableCellWithIdentifier:kCellID1 forIndexPath:indexPath];
            cell.delegate = self;
            ...
            CCReciveMsgCell *cell = [tableView dequeueReusableCellWithIdentifier:kCellID0 forIndexPath:indexPath];
            cell.delegate = self;
